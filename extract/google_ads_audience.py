...
AUDIENCE_TABLE_NAME = os.getenv("AUDIENCE_TABLE_NAME")

# --- GAQL Query for AUDIENCE---
QUERY_TEMPLATE = """
SELECT
  segments.date,
  customer.id,
...
def extract_audience_data(customer_id: str, start_date: str, end_date: str, ads_client):
    """Extracts AUDIENCE performance data from Google Ads for a specific account and date range."""
    service = ads_client.get_service("GoogleAdsService")
    ...
    for batch in response:
        for row in batch.results:
            ...
            # --- PARSE THE CRITERION ID from the resource_name ---
            # The resource_name looks like:
            # customers/12345/adGroupAudienceViews/67890~555123
            # We want the last part (555123)
            resource_name = getattr(row.ad_group_audience_view, "resource_name", "")
            audience_criterion_id = resource_name.split('~')[-1]
            # --- END PARSING ---
            ...
            rows.append({
                ...
                "audience_criterion_id": audience_criterion_id,
...
def get_last_loaded_date(bq_client):
    table_id = f"{PROJECT_ID}.{RAW_DATASET_NAME}.{AUDIENCE_TABLE_NAME}"
...
def load_to_bigquery(df: pd.DataFrame, start_date: str, end_date: str, account_name: str, account_id: str, bq_client):
    ...
    job.result()
    print(f"Loaded {len(df)} rows for {account_name} ({account_id}) into {RAW_DATASET_NAME}.{AUDIENCE_TABLE_NAME}")
...
def main():
    ...
    for account in child_accounts:
        ...
        try:
            df = extract_audience_data(customer_id, start_date, end_date, ads_client)
            if df.empty:
...
#         customer_id = str(account["id"])
#         account_name = account["name"]
#         print(f"\nExtracting AUDIENCE for account: {account_name} ({customer_id})")
#
...
#
#             print(f"Extracting AUDIENCE {start_date} → {end_date}")
#             try:
#                 df = extract_audience_data(customer_id, start_date, end_date, ads_client)
#
#                 if df.empty:
#                     print(f"No AUDIENCE data for {yr} in {account_name}")
#                     continue
#
#                 print(f"Extracted AUDIENCE {len(df)} rows for {yr} ({account_name})")
#                 load_to_bigquery(df, bq_client)
#
#             except Exception as e:
#                 print(f"Failed AUDIENCE for {account_name} ({customer_id}): {e}")
#
#
# if __name__ == "__main__":
#     main()
