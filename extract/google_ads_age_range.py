...
AGE_TABLE_NAME = os.getenv("AGE_TABLE_NAME")

# --- GAQL Query for AGE RANGE---
QUERY_TEMPLATE = """
SELECT
  segments.date,
  customer.id,
...
def extract_age_data(customer_id: str, start_date: str, end_date: str, ads_client):
    """Extracts AGE RANGE performance data from Google Ads for a specific account and date range."""
    service = ads_client.get_service("GoogleAdsService")
...
def main():
    ...
    for account in child_accounts:
        ...
        try:
            df = extract_age_data(customer_id, start_date, end_date, ads_client)
            if df.empty:
