...
def extract_ad_creative_dim(customer_id: str):
...
# --- LOAD TO BIGQUERY
def load_ad_dim_to_bigquery(df: pd.DataFrame):
...
def main():
    ...
    for account in child_accounts:
        ...
        try:
            ...
            df_dim = extract_ad_creative_dim(customer_id)
            ...
            print(f"Failed Ad Performance Dimension for {account_name} ({customer_id}): {e}")

    # --- After looping, combine, deduplicate, and load ---
    if not all_ad_dfs:
        print("No Ad Performance dimension data found in any account. Exiting.")
        return
    ...
    try:
        ...
        load_ad_dim_to_bigquery(master_df_deduped)
