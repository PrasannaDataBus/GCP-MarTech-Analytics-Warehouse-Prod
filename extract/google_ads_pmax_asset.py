...
PMAX_ASSET_TABLE_NAME = os.getenv("PMAX_ASSET_TABLE_NAME")

# --- GAQL Query for PMAX ASSET---
QUERY_TEMPLATE = """
SELECT
  segments.date,
  customer.id,
...
WHERE segments.date BETWEEN '{start_date}' AND '{end_date}'
    AND campaign.advertising_channel_type IN ('PERFORMANCE_MAX', 'SMART', 'LOCAL')
"""
...
def extract_pmax_data(customer_id: str, start_date: str, end_date: str, ads_client):
    """Extracts Performance Max ASSET data from Google Ads for a specific account and date range."""
    service = ads_client.get_service("GoogleAdsService")
    ...
    for batch in response:
        for row in batch.results:
            rows.append({
                ...
                "asset_text": getattr(row.asset.text_asset, "text", None), # For Headlines/Descriptions
...
def get_last_loaded_date(bq_client):
    table_id = f"{PROJECT_ID}.{RAW_DATASET_NAME}.{PMAX_ASSET_TABLE_NAME}"
...
def load_to_bigquery(df: pd.DataFrame, start_date: str, end_date: str, account_name: str, account_id: str, bq_client):
    ...
    job.result()
    print(f"Loaded {len(df)} rows for {account_name} ({account_id}) into {RAW_DATASET_NAME}.{PMAX_ASSET_TABLE_NAME}")
...
def main():
    ...
    for account in child_accounts:
        ...
        try:
            # Step 1: Extract
            df = extract_pmax_data(customer_id, start_date, end_date, ads_client)
            if df.empty:
...
#         customer_id = str(account["id"])
#         account_name = account["name"]
#         print(f"\nExtracting PMAX ASSET DATA for account: {account_name} ({customer_id})")
#
...
#
#             print(f"Extracting PMAX ASSET DATA {start_date} → {end_date}")
#             try:
#                 df = extract_pmax_data(customer_id, start_date, end_date, ads_client)
#
#                 if df.empty:
#                     print(f"No PMAX ASSET DATA data for {yr} in {account_name}")
#                     continue
#
#                 print(f"Extracted PMAX ASSET DATA {len(df)} rows for {yr} ({account_name})")
#                 load_to_bigquery(df, bq_client)
#
#             except Exception as e:
#                 print(f"Failed PMAX ASSET DATA for {account_name} ({customer_id}): {e}")
#
#
# if __name__ == "__main__":
#     main()
