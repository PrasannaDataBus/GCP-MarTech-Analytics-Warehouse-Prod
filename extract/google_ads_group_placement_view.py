...
PLACEMENT_TABLE_NAME = os.getenv("PLACEMENT_TABLE_NAME")

# --- GAQL Query for PLACEMENT VIEW---
QUERY_TEMPLATE = """
SELECT
  segments.date,
  customer.id,
...
def extract_placement_data(customer_id: str, start_date: str, end_date: str, ads_client):
    """Extracts PLACEMENT VIEW data from Google Ads for a specific account and date range."""
    service = ads_client.get_service("GoogleAdsService")
    ...
    for batch in response:
        for row in batch.results:
            rows.append({
                ...
                "placement_resource_name": row.group_placement_view.placement,
                "placement_display_name": row.group_placement_view.display_name,
...
def get_last_loaded_date(bq_client):
    table_id = f"{PROJECT_ID}.{RAW_DATASET_NAME}.{PLACEMENT_TABLE_NAME}"
...
def load_to_bigquery(df: pd.DataFrame, start_date: str, end_date: str, account_name: str, account_id: str, bq_client):
    ...
    job_config = bigquery.LoadJobConfig(write_disposition="WRITE_APPEND",
                                        schema = [
                                            ...
                                            bigquery.SchemaField("placement_resource_name", "STRING"),
                                            bigquery.SchemaField("placement_display_name", "STRING"),
    ...
    job.result()
    print(f"Loaded {len(df)} rows for {account_name} ({account_id}) into {RAW_DATASET_NAME}.{PLACEMENT_TABLE_NAME}")
...
def main():
    ...
    for account in child_accounts:
        ...
        try:
            df = extract_placement_data(customer_id, start_date, end_date, ads_client)
            if df.empty:
...
#         customer_id = str(account["id"])
#         account_name = account["name"]
#         print(f"\nExtracting PLACEMENT VIEW DATA for account: {account_name} ({customer_id})")
#
...
#
#             print(f"Extracting PLACEMENT VIEW DATA {start_date} → {end_date}")
#             try:
#                 df = extract_placement_data(customer_id, start_date, end_date, ads_client)
#
#                 if df.empty:
#                     print(f"No PLACEMENT VIEW DATA data for {yr} in {account_name}")
#                     continue
#
#                 print(f"Extracted PLACEMENT VIEW DATA {len(df)} rows for {yr} ({account_name})")
#                 load_to_bigquery(df, bq_client)
#
#             except Exception as e:
#                 print(f"Failed PLACEMENT VIEW DATA for {account_name} ({customer_id}): {e}")
#
#
# if __name__ == "__main__":
#     main()
