...
SEARCH_TERMS_TABLE_NAME = os.getenv("SEARCH_TERMS_TABLE_NAME")
...
SELECT
  ...
  segments.keyword.ad_group_criterion, -- This field contains the keyword resource name
  metrics.impressions,
  metrics.clicks,
  metrics.ctr,
  metrics.average_cpc,
...
def extract_search_terms_data(customer_id: str, start_date: str, end_date: str, ads_client):
    """Extracts SEARCH TERMS data from Google Ads for a specific account and date range."""
    service = ads_client.get_service("GoogleAdsService")
    ...
    for batch in response:
        for row in batch.results:
            # --- PARSE THE KEYWORD ID from the resource_name ---
            # The resource_name looks like:
            # customers/12345/adGroupCriteria/67890~555123
            # We want the last part (555123)
            resource_name = getattr(row.segments.keyword, "ad_group_criterion", "")
            keyword_id = resource_name.split('~')[-1] if resource_name else None
            # --- END PARSING ---
            ...
            rows.append({
                ...
                "keyword_id": keyword_id, # This links to your keyword_raw table
...
def get_last_loaded_date(bq_client):
    table_id = f"{PROJECT_ID}.{RAW_DATASET_NAME}.{SEARCH_TERMS_TABLE_NAME}"
...
def load_to_bigquery(df: pd.DataFrame, start_date: str, end_date: str, account_name: str, account_id: str, bq_client):
    ...
    job_config = bigquery.LoadJobConfig(write_disposition="WRITE_APPEND",
                                        schema = [
                                            ...
                                            bigquery.SchemaField("keyword_id", "STRING"),  # Parsed from resource_name
    ...
    job.result()
    print(f"Loaded {len(df)} rows for {account_name} ({account_id}) into {RAW_DATASET_NAME}.{SEARCH_TERMS_TABLE_NAME}")
...
def main():
    ...
    for account in child_accounts:
        ...
        try:
            df = extract_search_terms_data(customer_id, start_date, end_date, ads_client)
            if df.empty:
...
#         customer_id = str(account["id"])
#         account_name = account["name"]
#         print(f"\nExtracting SEARCH TERMS for account: {account_name} ({customer_id})")
#
...
#
#             print(f"Extracting SEARCH TERMS {start_date} → {end_date}")
#             try:
#                 df = extract_search_terms_data(customer_id, start_date, end_date, ads_client)
#
#                 if df.empty:
#                     print(f"No SEARCH TERMS data for {yr} in {account_name}")
#                     continue
#
#                 print(f"Extracted SEARCH TERMS {len(df)} rows for {yr} ({account_name})")
#                 load_to_bigquery(df, bq_client)
#
#             except Exception as e:
#                 print(f"Failed SEARCH TERMS for {account_name} ({customer_id}): {e}")
#
#
# if __name__ == "__main__":
#     main()
