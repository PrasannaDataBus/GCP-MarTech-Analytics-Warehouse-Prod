...
SCHEDULE_TABLE_NAME = os.getenv("SCHEDULE_TABLE_NAME")

# --- GAQL Query for SCHEDULE---
QUERY_TEMPLATE = """
SELECT
  segments.date,
...
def extract_schedule_data(customer_id: str, start_date: str, end_date: str, ads_client):
    """Extracts SCHEDULING data from Google Ads for a specific account and date range."""
    service = ads_client.get_service("GoogleAdsService")
...
def get_last_loaded_date(bq_client):
    table_id = f"{PROJECT_ID}.{RAW_DATASET_NAME}.{SCHEDULE_TABLE_NAME}"
...
def load_to_bigquery(df: pd.DataFrame, start_date: str, end_date: str, account_name: str, account_id: str, bq_client):
    ...
    job.result()
    print(f"Loaded {len(df)} rows for {account_name} ({account_id}) into {RAW_DATASET_NAME}.{SCHEDULE_TABLE_NAME}")
...
def main():
    ...
    for account in child_accounts:
        ...
        try:
            df = extract_schedule_data(customer_id, start_date, end_date, ads_client)
            if df.empty:
...
#         customer_id = str(account["id"])
#         account_name = account["name"]
#         print(f"\nExtracting SCHEDULING DATA for account: {account_name} ({customer_id})")
#
...
#
#             print(f"Extracting SCHEDULING DATA {start_date} → {end_date}")
#             try:
#                 df = extract_schedule_data(customer_id, start_date, end_date, ads_client)
#
#                 if df.empty:
#                     print(f"No SCHEDULING DATA data for {yr} in {account_name}")
#                     continue
#
#                 print(f"Extracted SCHEDULING DATA {len(df)} rows for {yr} ({account_name})")
#                 load_to_bigquery(df, bq_client)
#
#             except Exception as e:
#                 print(f"Failed SCHEDULING DATA for {account_name} ({customer_id}): {e}")
#
#
# if __name__ == "__main__":
#     main()
