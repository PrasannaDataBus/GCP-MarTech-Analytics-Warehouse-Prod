...
from google_ads_pmax_asset_dim import main as google_ads_main
...
with DAG(
    dag_id="google_ads_pmax_asset_dim_raw_dag",
    default_args=default_args,
    description="Incrementally extract and load Google Ads PMAX ASSET DIMESION data into BigQuery",
    schedule_interval="0 6 * * 1",  # Every Monday at 06:00
    start_date=datetime(2025, 11, 1),
    catchup=False,
    tags=["google_ads", "bigquery", "incremental", "prod", "PMAX ASSET DIMESION"],
) as dag:

    run_incremental_extraction = PythonOperator(
        task_id="extract_and_load_google_ads_pmaxasset_dim_raw_data",
