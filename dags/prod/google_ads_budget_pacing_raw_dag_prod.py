...
from google_ads_budget_pacing import main as google_ads_main
...
with DAG(
    dag_id="google_ads_budget_pacing_raw_dag",
    default_args=default_args,
    description="Incrementally extract and load BUDGET PACING data into BigQuery",
    schedule_interval="0 6 * * *",  # Runs daily at 6 AM
    start_date=datetime(2025, 11, 1),
    catchup=False,
    tags=["google_ads", "bigquery", "incremental", "prod", "BUDGET PACING"],
) as dag:

    run_incremental_extraction = PythonOperator(
        task_id="extract_and_load_google_ads_budget_pacing_raw_data",
